...
class MyPlanner:
    ...
    def plan(self, start, goal):
        ...
        while len(openSet):
            ...
            g_score = current_node.cost + 0.5 # cost to get to next is 0.5 in all directions
            for nb in neighbors:
                # if the neighbor is not visited before
                # add to the priority queue
                if tuple(nb) not in visited:
                    new_node = Node(nb, g_score, current_node)
    ...
    def explore_neighbors(self, node):
        ...
        dR = np.delete(dR,[0,1,2,3,5,6,7,8,9,11,13,15,17,18,19,20,21,23,24,25,26],axis=1)
        dR = dR / 2
    ...
    def is_valid(self, point1, point2):
        '''
        Check if the path from point1 to point2 is valid
        '''
        # if point out of boundary
        if( point2[0] <= self.boundary[0,0] or point2[0] >= self.boundary[0,3] or \
            point2[1] <= self.boundary[0,1] or point2[1] >= self.boundary[0,4] or \
            point2[2] <= self.boundary[0,2] or point2[2] >= self.boundary[0,5] or \
            self.check_collision(point2) ):
            return False
        
        # if line go through AABB
        for i in range(3):
            direction = point2 - point1
            tmp = point1 + (i+1)*direction/3
    ...
    def check_collision(self, point):
        ...
        for k in range(self.blocks.shape[0]):
            if( point[0] >= self.blocks[k,0]-0.1 and point[0] <= self.blocks[k,3]+0.1 and \
                point[1] >= self.blocks[k,1]-0.1 and point[1] <= self.blocks[k,4]+0.1 and \
                point[2] >= self.blocks[k,2]-0.1 and point[2] <= self.blocks[k,5]+0.1 ):
                return True
        return False
